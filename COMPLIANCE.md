...
| Блок | Что входит | Compliance-примечание |
|---|---|---|
| 👤 **Пользователи** | Профиль, лента, подписки, лайки, комментарии, репосты, чаты, группы, блоги | UGC модерация обязательна (Apple §1.2, Google UGC, DSA) |
| 🏢 **Бизнес-профили** | Специалисты, услуги, салоны/студии, курсы, продукты, B2B | Privacy Policy должна описать обработку данных бизнес-аккаунтов |
| 👥 **Сообщества** | Группы по интересам, блоги, тематические страницы | Community Guidelines обязательны |
| 🎯 **Категории** | Питание, уход/красота, спорт, окружение, режим дня, ментальное здоровье | Контент = UGC, модерация health-claims НЕ обязательна для соцсети (мы НЕ медицинский сервис) |
| 🎁 **Бонусы / Gamification** | Баллы за активность, обмен на подарки | НЕ является финансовой деятельностью — это loyalty program. Нет лицензии не нужно (аналог: Reddit karma, Duolingo gems) |
| 🤖 **AI** | Рекомендации, поиск, подбор контента | На текущем этапе: [EU AI Act](https://eur-lex.europa.eu/eli/reg/2024/1689/oj) классифицирует рекомендательные системы как «минимальный риск» → обязательства ограничены прозрачностью (раскрыть что используется AI). Добавить в Privacy Policy |

> **Вывод:** BestMe = **социальная сеть с UGC**. Все compliance-требования этого документа полностью покрывают платформу. Специальные лицензии (финансовые, медицинские, крипто) на текущем этапе **НЕ нужны** — бонусная система = loyalty points, AI = рекомендации (минимальный риск), контент = UGC (не медицинские рекомендации).
...
>
> ✅ = Проверено. Наших 8 пунктов достаточно для запуска. Дополнительные доработки НЕ нужны.
