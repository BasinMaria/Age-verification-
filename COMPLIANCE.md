...
1. [🏪 ГЛАВНОЕ — Публикация в магазины (App Store + Google Play)](#1--главное--публикация-в-магазины-app-store--google-play)
   ...
   - [📁 Структура проекта и файлов](#-структура-проекта-и-файлов--что-проверяют-и-за-что-отказывают)
   - [🖼️ Метаданные и листинг магазина](#-метаданные-и-листинг-магазина--что-обязательно-заполнить)
   - [🚫 Неочевидные причины отказа](#-неочевидные-причины-отказа--что-часто-пропускают)
   - [👨‍💻👩‍🎨🧪 Что делать перед подачей — по ролям](#-что-делать-перед-подачей--по-ролям)
...
#### 📁 Структура проекта и файлов — что проверяют и за что отказывают

> **Да, файловая структура проекта МОЖЕТ стать причиной отказа.** Apple и Google проверяют бинарник, метаданные, конфигурацию.

##### 🍎 Apple — требования к проекту и бинарнику

| # | Что проверяется | Требование | Что будет если нарушить |
|---|---|---|---|
| 1 | **Bundle ID** (`CFBundleIdentifier`) | Должен совпадать с App Store Connect. Формат: `com.company.appname`. Один Bundle ID = одно приложение | Бинарник не пройдёт валидацию при загрузке (Upload Error) |
| 2 | **Provisioning Profile + Code Signing** | Подпись **Distribution** сертификатом (не Development). Provisioning profile должен включать все capabilities (Push, Sign in with Apple и т.д.) | Xcode покажет ошибку при Archive. Если загрузить с неправильным профилем → отказ при Processing |
| 3 | **Info.plist** — полнота и корректность | Все обязательные ключи: `CFBundleShortVersionString`, `CFBundleVersion`, Purpose Strings для разрешений, `LSRequiresIPhoneOS`, `UILaunchStoryboardName` | Автоматический отказ при загрузке или при ревью |
| 4 | **App Icons** — полный набор | Должны быть **ВСЕ** размеры иконок (1024×1024 для Store + все размеры для устройств). С Xcode 15 — достаточно одной 1024×1024 (автогенерация). Иконка НЕ должна содержать alpha-канал | Upload Error: «Missing required icon» или «Icon contains alpha channel» |
| 5 | **Launch Screen** | Обязателен. Можно Storyboard или SwiftUI. **НЕ должен** быть статичной картинкой с рекламой или текстом — только бренд/лого или пустой экран | Apple §4.0: «Apps that display a launch screen that appears to be advertising» → отказ |
| 6 | **Архитектура arm64** | С 2023 — только **arm64**. Нельзя включать i386, armv7, x86_64 (для iOS). Xcode 15+ по умолчанию собирает правильно | Upload Error: «Invalid binary — unsupported architecture» |
| 7 | **Minimum Deployment Target** | Рекомендуется не ниже iOS 16 (2026). Apple не отклоняет за старый target, но новые API не будут доступны. Beta тестирование на последней iOS обязательно | Не ошибка, но ограничивает функции и аудиторию |
| 8 | **PrivacyInfo.xcprivacy** | Файл манифеста конфиденциальности. Должен быть в **главном таргете** и в каждом SDK/framework который использует Required Reason API | Автоматическое предупреждение → переходит в отказ |
| 9 | **Entitlements** | Capabilities в Xcode (Push Notifications, Sign in with Apple, Associated Domains) должны совпадать с App ID в Developer Portal | Крашит функции при запуске → ревьюер видит краш → отказ |
| 10 | **Размер бинарника** | Лимит Apple: **4 ГБ** общий размер. Для загрузки через мобильную сеть: приложение > 200 МБ требует Wi-Fi (с iOS 13 можно настроить лимит) | Не отказ, но пользователи на мобильном интернете не смогут скачать. Оптимизировать: App Thinning, On-Demand Resources |

##### 🤖 Android — требования к проекту и бинарнику

| # | Что проверяется | Требование | Что будет если нарушить |
|---|---|---|---|
| 1 | **Формат AAB** (Android App Bundle) | С 2021 Google **требует AAB** (не APK) для новых приложений в Play Store | Upload Error: «Upload a valid Android App Bundle» |
| 2 | **applicationId** в `build.gradle` | Уникальный ID пакета (`com.company.appname`). Должен совпадать с Play Console | Бинарник не пройдёт загрузку |
| 3 | **versionCode** (число) и **versionName** (строка) | `versionCode` должен быть **больше** предыдущей загруженной версии. Начинать с 1, увеличивать при каждом релизе | Upload Error: «Version code already exists» |
| 4 | **Signing Key** | App Signing Key через Google Play App Signing (рекомендуется) или собственный keystore. **НЕЛЬЗЯ** терять ключ — без него невозможно обновить приложение | Если потерял ключ → создавать новое приложение с нуля (новый applicationId) |
| 5 | **AndroidManifest.xml** — полнота | Все permissions **обоснованы**, все activities/services/receivers объявлены. Нет `android:debuggable="true"` в release | `debuggable=true` → автоматический отказ. Лишние permissions → ручной отказ |
| 6 | **targetSdkVersion** | Должен быть актуальным (2025 = 35). Google каждый год повышает требование | Нельзя загрузить обновление если targetSdk ниже минимального |
| 7 | **minSdkVersion** | Рекомендуется ≥ 24 (Android 7.0). Можно ниже, но <21 → проблемы с безопасностью | Не отказ, но ограничивает аудиторию |
| 8 | **ProGuard / R8** (обфускация) | Рекомендуется. Уменьшает размер APK/AAB + защищает код | Не обязательно, но без обфускации — код легко декомпилировать |
| 9 | **64-bit support** | С 2019 Google **требует** 64-bit нативные библиотеки (`arm64-v8a`). Если есть 32-bit `.so` без 64-bit пары → отказ | Upload Error: «This release is not compliant with the Google Play 64-bit requirement» |
| 10 | **Размер AAB** | Лимит: **150 МБ** для base APK (из AAB). Для большего — использовать Play Asset Delivery (до 2 ГБ) | Upload Error если base > 150 МБ |

---

#### 🖼️ Метаданные и листинг магазина — что ОБЯЗАТЕЛЬНО заполнить

> **Неполные или некорректные метаданные = отказ.** Ревьюер проверяет: скриншоты, описание, аккаунт для тестирования.

| # | Что нужно | Apple | Google | Что будет если нарушить |
|---|---|---|---|---|
| 1 | **Скриншоты** | Минимум 3 скриншота. Для каждого размера экрана (6.7", 6.5", 5.5", iPad если Universal). Формат: PNG или JPEG, без alpha | Минимум 2 скриншота (до 8). Для phone + если есть tablet/Chromebook | Без скриншотов → нельзя отправить на ревью. Скриншоты НЕ соответствуют реальному приложению → отказ (Apple §2.3.1) |
| 2 | **Скриншоты = реальное приложение** | Скриншоты должны показывать **реальный UI** приложения. Можно добавить маркетинговые рамки/текст, но основной контент = настоящие экраны | Аналогично | «Screenshots do not sufficiently reflect the app in use» → отказ |
| 3 | **Описание приложения** | Не может быть пустым, placeholder, или «Lorem ipsum». Должно точно описывать что приложение делает | Аналогично | Apple §2.3: «App descriptions should accurately describe the app» → отказ |
| 4 | **Тестовый аккаунт для ревьюера** | ⚠️ **КРИТИЧЕСКИ ВАЖНО:** Если приложение требует логин → **ОБЯЗАТЕЛЬНО** указать demo-аккаунт (email + пароль) в «App Review Information» в App Store Connect | В Google Play Console → «Review information» | **#1 причина отказов.** Ревьюер не может войти → «We were unable to review your app as we could not sign in» → отказ. Аккаунт должен РЕАЛЬНО РАБОТАТЬ |
| 5 | **Возрастной рейтинг** | Заполнить Content Descriptions → получить 17+ или 18+ | Заполнить Content Rating Questionnaire | Неправильный рейтинг → отказ или изменение рейтинга |
| 6 | **Категория** | Выбрать подходящую (Social Networking / Health & Fitness / Lifestyle). НЕ ставить Kids | Аналогично | Неправильная категория → отказ |
| 7 | **Privacy Policy URL** | **ОБЯЗАТЕЛЬНО.** Рабочая ссылка на веб-страницу | **ОБЯЗАТЕЛЬНО.** Рабочая ссылка | Нет ссылки или ссылка не работает → мгновенный отказ |
| 8 | **Support URL** | **Обязательно.** Ссылка на страницу поддержки / контактный email | Обязательно | Нет → отказ |
| 9 | **Название приложения** | НЕ должно содержать: «beta», «test», «demo», «free», цены, чужие бренды | Аналогично | Apple §2.3.7: «App names should not include price» / «beta» → отказ |
| 10 | **Ключевые слова** (Apple) | Нельзя использовать чужие бренды (Instagram, TikTok, Facebook) как ключевые слова | Нет отдельного поля keywords, но описание сканируется | Apple §2.3.7: «Use of third-party trademarks in keyword metadata» → отказ |

---

#### 🚫 Неочевидные причины отказа — что ЧАСТО пропускают

> **Эти причины НЕ связаны с кодом напрямую, но ломают публикацию.**

| # | Причина отказа | Описание | Как избежать |
|---|---|---|---|
| 1 | **Placeholder-контент** в приложении | Lorem ipsum, «TODO», «Test», шаблонные иконки, пустые экраны | Перед подачей пройти ВСЁ приложение — каждый экран. Удалить весь тестовый контент |
| 2 | **Нерабочие ссылки** | Privacy Policy, Terms of Service, Support — ссылки ведут на 404 или «Under Construction» | Проверить ВСЕ ссылки из приложения. Страницы должны быть уже опубликованы |
| 3 | **Неполная функциональность** | Кнопки которые ведут в никуда, экраны «Coming Soon», пустые разделы | Apple §2.1: «Apps that are not fully functional will be rejected.» Каждая видимая кнопка = рабочая функция |
| 4 | **Отсутствие тестового аккаунта** | Ревьюер не может войти / зарегистрироваться | ВСЕГДА указывать рабочий demo-аккаунт. Проверить что он работает ПЕРЕД отправкой |
| 5 | **«Ревью-режим» / A-B тесты** | Приложение ведёт себя по-разному при ревью и для пользователей | Apple мониторит сеть и поведение. Обнаружение = бан аккаунта разработчика |
| 6 | **Упоминание других платформ** | «Скачайте также в Google Play» в iOS-приложении или наоборот | Apple §2.3.1: нельзя упоминать конкурентные платформы. Убрать ВСЕ упоминания |
| 7 | **Логотипы/бренды без разрешения** | Использование логотипа Apple, Google, Instagram и т.д. в UI или маркетинге | Удалить все чужие бренды. Разрешены только стандартные «Sign in with Apple/Google» кнопки по гайдлайнам |
| 8 | **Приложение не работает без сервера** | Бэкенд упал во время ревью | Убедиться что бэкенд стабилен. Apple ревью может быть в ЛЮБОЕ время (24/7, любой часовой пояс) |
| 9 | **Debug/dev индикаторы** | Console.log вывод на экран, debug-меню доступное пользователю, test-баннеры рекламы | Убрать ВСЕ debug-элементы из release-сборки |
| 10 | **Устаревшие скриншоты** | Скриншоты показывают старый дизайн / несуществующие экраны | Обновлять скриншоты при каждом визуальном изменении |
| 11 | **Нет контента для ревьюера** | Лента пустая, нечего смотреть | Наполнить приложение seed-данными (посты, профили, блоги) чтобы ревьюер видел реальный опыт |
| 12 | **Авторские права на контент** | Использование чужих фото, музыки, видео как seed-контент | Только свой контент или контент с лицензией (CC0, stock photos с лицензией) |

---

#### 👨‍💻👩‍🎨🧪 ЧТО ДЕЛАТЬ ПЕРЕД ПОДАЧЕЙ — по ролям

> **Каждая роль в команде имеет свой чеклист.** Выполнить ВСЁ перед отправкой в магазин.

##### 👨‍💻 Программисты (Developers) — чеклист перед подачей

```
╔══════════════════════════════════════════════════════════════════════════════╗
║            ЧЕКЛИСТ ПРОГРАММИСТА — ПЕРЕД ПОДАЧЕЙ НА РЕВЬЮ                    ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  СБОРКА И КОНФИГУРАЦИЯ:                                                      ║
║  ☐ Release-сборка (НЕ debug). android:debuggable=false                       ║
║  ☐ Правильный Bundle ID / applicationId                                      ║
║  ☐ Версия и build number увеличены                                           ║
║  ☐ Distribution certificate + правильный Provisioning Profile (iOS)          ║
║  ☐ App Signing Key настроен (Android)                                        ║
║  ☐ AAB формат (Android), Archive (iOS)                                       ║
║  ☐ Минимум arm64 архитектура                                                 ║
║  ☐ targetSdkVersion актуальная (Android)                                     ║
║  ☐ Xcode + iOS SDK актуальная версия (iOS)                                   ║
║                                                                              ║
║  КОД:                                                                        ║
║  ☐ Нет вызовов Private API                                                   ║
║  ☐ Нет eval() / dlopen / загрузки кода                                       ║
║  ☐ Нет deprecated API без fallback                                           ║
║  ☐ Все console.log / print / debug-вывод УБРАНЫ из release                   ║
║  ☐ Нет TODO / FIXME / HACK комментариев в видимых частях                     ║
║  ☐ ProGuard/R8 включён (Android release)                                     ║
║  ☐ PrivacyInfo.xcprivacy заполнен (iOS)                                      ║
║  ☐ Info.plist: Purpose String для КАЖДОГО разрешения (iOS)                   ║
║  ☐ AndroidManifest.xml: только нужные permissions (Android)                  ║
║  ☐ Нет hardcoded API keys / secrets в коде (только env / Vault)              ║
║                                                                              ║
║  ФУНКЦИОНАЛЬНОСТЬ:                                                           ║
║  ☐ ВСЕ кнопки работают (нет «Coming Soon» / «TODO»)                          ║
║  ☐ Регистрация и вход работают (email + соцсети)                             ║
║  ☐ DOB-форма работает и блокирует < 18                                       ║
║  ☐ Удаление аккаунта работает                                                ║
║  ☐ Push notifications настроены (APNs + FCM сертификаты актуальны)           ║
║  ☐ Deep links / Universal Links работают                                     ║
║  ☐ Бэкенд (Supabase) стабилен и доступен 24/7                               ║
║                                                                              ║
║  ДАННЫЕ:                                                                     ║
║  ☐ Privacy Nutrition Label / Data Safety заполнены                           ║
║  ☐ DOB НЕ попадает в аналитику (только age_bracket)                          ║
║  ☐ Export Compliance заполнен (iOS)                                           ║
║  ☐ Тестовый аккаунт для ревьюера создан и РАБОТАЕТ                           ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
```

##### 👩‍🎨 Дизайнеры (Designers) — чеклист перед подачей

```
╔══════════════════════════════════════════════════════════════════════════════╗
║            ЧЕКЛИСТ ДИЗАЙНЕРА — ПЕРЕД ПОДАЧЕЙ НА РЕВЬЮ                       ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  ИКОНКА ПРИЛОЖЕНИЯ:                                                          ║
║  ☐ 1024×1024 PNG без alpha-канала (прозрачности)                            ║
║  ☐ Иконка читается в маленьком размере (29×29, 40×40)                       ║
║  ☐ НЕ копирует системные иконки iOS/Android                                  ║
║  ☐ НЕ содержит чужих брендов / логотипов                                     ║
║  ☐ НЕ содержит слов «beta» / «test» / «new» / «free»                        ║
║                                                                              ║
║  СКРИНШОТЫ ДЛЯ МАГАЗИНА:                                                     ║
║  ☐ Скриншоты = РЕАЛЬНЫЕ экраны приложения                                    ║
║  ☐ Apple: подготовлены для КАЖДОГО размера (6.7", 6.5", 5.5" + iPad)        ║
║  ☐ Google: подготовлены для phone (min 2, max 8) + tablet (если есть)       ║
║  ☐ Нет placeholder-текста (Lorem ipsum, «test», «example@mail.com»)         ║
║  ☐ Нет чужих брендов на скриншотах                                           ║
║  ☐ Seed-контент выглядит реалистично                                         ║
║  ☐ Feature Graphic 1024×500 (Google Play — обязательно)                      ║
║                                                                              ║
║  UI/UX ПРОВЕРКИ:                                                             ║
║  ☐ Safe Area: контент НЕ обрезается за notch / Dynamic Island               ║
║  ☐ Все размеры экранов: SE → Pro Max (iOS), 5"→ tablet (Android)            ║
║  ☐ Dark Mode: текст читается, контрастность ≥ 4.5:1 (WCAG AA)              ║
║  ☐ Dynamic Type (iOS): UI не ломается при крупном шрифте                     ║
║  ☐ Edge-to-edge (Android 15+): контент не за system bars                     ║
║  ☐ RTL (если AR/HE): layout корректно зеркалится                            ║
║  ☐ Нативные элементы: UINavigationBar, UITabBar или их аналоги (iOS)        ║
║  ☐ Material Design 3 базовые принципы (Android)                              ║
║  ☐ Launch Screen: без рекламы, без текста (только бренд/лого)               ║
║  ☐ Нет пустых экранов / «Coming Soon» / placeholder-иконок                  ║
║                                                                              ║
║  ACCESSIBILITY (ДОСТУПНОСТЬ):                                                ║
║  ☐ VoiceOver labels на основных кнопках и элементах (iOS)                   ║
║  ☐ TalkBack contentDescription на основных элементах (Android)               ║
║  ☐ Контрастность текста достаточная (тёмный текст на светлом фоне и наоборот)║
║  ☐ Кнопки достаточного размера (минимум 44×44 pt iOS, 48×48 dp Android)     ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
```

##### 🧪 Тестировщики (QA) — чеклист перед подачей

```
╔══════════════════════════════════════════════════════════════════════════════╗
║           ЧЕКЛИСТ ТЕСТИРОВЩИКА (QA) — ПЕРЕД ПОДАЧЕЙ НА РЕВЬЮ                ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  УСТРОЙСТВА (тестировать НА РЕАЛЬНЫХ устройствах, не только эмулятор):       ║
║  ☐ iPhone SE (маленький экран, кнопка Home)                                  ║
║  ☐ iPhone 15/16 Pro Max (большой экран, Dynamic Island)                      ║
║  ☐ iPad (если Universal app)                                                 ║
║  ☐ Android: бюджетный телефон (2 ГБ RAM, маленький экран)                   ║
║  ☐ Android: флагман (большой экран, последний Android)                       ║
║  ☐ Android: tablet / foldable (если поддерживается)                          ║
║                                                                              ║
║  ОСНОВНЫЕ ПОТОКИ (каждый пройти от начала до конца):                         ║
║  ☐ Регистрация по email (email + имя + пароль + DOB) → успех                ║
║  ☐ Регистрация по email → DOB < 18 → отказ → retry → мгновенный отказ      ║
║  ☐ Вход через Google → DOB-экран → успех                                     ║
║  ☐ Вход через Facebook → DOB-экран → успех                                   ║
║  ☐ Вход через Apple Sign-In → DOB-экран → успех                             ║
║  ☐ Выход из аккаунта → повторный вход                                        ║
║  ☐ Удаление аккаунта → подтверждение → email уведомление                    ║
║  ☐ Создание поста (текст + фото) → публикация → виден в ленте               ║
║  ☐ Жалоба на пост → подтверждение                                            ║
║  ☐ Блокировка пользователя → контент скрыт                                   ║
║  ☐ Push notification → получена → тап → правильный экран                     ║
║                                                                              ║
║  ПОГРАНИЧНЫЕ СЛУЧАИ (edge cases):                                            ║
║  ☐ Нет интернета → показывается сообщение об ошибке (не краш)               ║
║  ☐ Медленный интернет (3G) → приложение не зависает                          ║
║  ☐ Сервер недоступен → graceful error (не белый экран)                       ║
║  ☐ Поворот экрана (если не залочен) → UI не ломается                         ║
║  ☐ Свернуть / развернуть приложение → состояние сохраняется                  ║
║  ☐ Несколько быстрых тапов на кнопку → нет дублирования действий            ║
║  ☐ Очень длинный текст в поле ввода → UI не ломается                        ║
║  ☐ Пустая лента (новый пользователь) → показывается пустое состояние        ║
║  ☐ Разрешение камеры/фото отклонено → показывается «Go to Settings»         ║
║  ☐ Разрешение push отклонено → приложение работает без push                  ║
║                                                                              ║
║  ПЕРЕД САМОЙ ПОДАЧЕЙ:                                                        ║
║  ☐ Тестовый аккаунт для ревьюера РАБОТАЕТ (проверить прямо сейчас)          ║
║  ☐ Все ссылки в приложении работают (Privacy Policy, Terms, Support)         ║
║  ☐ Нет placeholder-контента (Lorem ipsum, test@test.com, «TODO»)            ║
║  ☐ Нет debug-элементов видимых пользователю                                  ║
║  ☐ Бэкенд стабилен (проверить что не упадёт во время ревью)                 ║
║  ☐ Seed-данные в ленте (посты, профили) — приложение не пустое               ║
║  ☐ Dark Mode — визуально проверен на всех основных экранах                   ║
║  ☐ Доступность (VoiceOver / TalkBack) — базовая проверка                    ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
```
...
#### ⏰ Сроки ревью — сколько ждать и что делать

| Магазин | Типичный срок ревью | Что делать если отказ |
|---|---|---|
| **Apple App Store** | 24–48 часов (первый раз может до 7 дней) | Читать rejection message → исправлять → отправлять повторно. Можно написать в Resolution Center для уточнений |
| **Google Play** | 1–3 дня (первый раз может до 7 дней) | Читать rejection email → исправлять → отправлять повторно. Можно подать апелляцию через Play Console |
| **Expedited Review (Apple)** | Можно запросить ускоренное ревью при критическом баге | Через App Store Connect → «Request an Expedited Review». Только для hot-fixes, не для первого запуска |

> **Совет:** Не отправлять на ревью в пятницу вечером. Лучшее время — **понедельник-среда**, утром по PST (Pacific Time), чтобы ревьюер Apple обработал в рабочие часы.
...
3. Показываем экран DOB (дата рождения):
   (⚠️ GeoIP проверка стран = Cloudflare на уровне инфраструктуры,
    ДО попадания в приложение. Подробнее → docs/BLOCKED-COUNTRIES.md)
   ┌─────────────────────────────────────┐
...
| **Подпись под toggle** | You can change this at any time in Settings → Privacy & Visibility | `change_in_settings_privacy_visibility` | Мелкий текст под toggle. **НЕ делать кликабельной ссылкой** на этом экране — это только информирование. Ссылка будет работать в Settings после входа |
...
> **Почему "acknowledge" а не "agree":** см. FAQ ниже. Коротко: "acknowledge" = «я ознакомлен» — безопаснее юридически. Оба варианта законны.
...
| Ссылка | Куда ведёт | Как открывается | Примечание |
|---|---|---|---|
| `[Privacy & Visibility]` (под toggle) | Страница настроек приватности **ВНУТРИ приложения** | **НЕ открывать** на этом экране. Текст = информирование: «потом сможешь поменять». Ссылка станет кликабельной **после** входа в приложение (в Settings). На Welcome Screen это просто текст-подсказка, НЕ активная ссылка | Если технически сложно — убрать скобки `[ ]` и оставить просто текст: *"You can change this at any time in Settings → Privacy & Visibility"* |
| `[Terms of Service]` (в footer) | Веб-страница ToS (bestme.app/terms) | Открывается в **in-app browser** (SafariViewController / Chrome Custom Tab) | Пользователь остаётся на Welcome Screen после закрытия |
| `[Privacy Policy]` (в footer) | Веб-страница PP (bestme.app/privacy) | Открывается в **in-app browser** | Пользователь остаётся на Welcome Screen после закрытия |

#### 📊 Что видно при Private vs Public профиле

> **Зачем этот раздел:** если все профили private — какой смысл соцсети? Ответ: private ≠ невидимый. Private = ограниченная видимость, а не полная изоляция.
...
> **Итого:** private профиль = как Instagram private account. Пользователь виден в поиске, может получать запросы на подписку, может писать и получать сообщения. Но его контент видят только те, кого он одобрил. Это НЕ «невидимка» — соцсеть работает нормально.

> **Бизнес-логика:** Большинство пользователей САМИ включат toggle на public, потому что хотят быть видимыми (это соцсеть). Но закон (CAADCA) требует, чтобы ПО УМОЛЧАНИЮ было private. Мы даём выбор — не навязываем.

#### ❓ Юридические ответы (FAQ)

**Q: Почему в тексте "private by default", а не "visible by default"?**
A: Закон [CAADCA (AB 2273)](https://leginfo.legislature.ca.gov/faces/billNavClient.xhtml?bill_id=202120220AB2273) **ТРЕБУЕТ** максимальные настройки приватности по умолчанию для пользователей до 18 лет. Поскольку Bestme = 18+, формально CAADCA не применяется напрямую. **НО**: Apple и Google ревьюеры проверяют privacy defaults для ВСЕХ приложений, и "visible by default" вызовет вопросы. Безопаснее = private by default + toggle для opt-in. Если когда-нибудь Bestme снизит возрастной лимит — CAADCA уже соблюдён.
...
A: ✅ **ДА**. GDPR Art. 25(2) ТРЕБУЕТ проинформировать пользователя о настройках приватности. Блокировка экрана = гарантия что пользователь проинформирован. Это стандартная практика (Pinterest, Instagram, TikTok делают то же самое). В интернете есть статьи что «нельзя блокировать» — это относится к **cookie-баннерам на сайтах** (ePrivacy Directive), а не к экранам onboarding в мобильных приложениях.
...
**Q: Почему "acknowledge" а не "agree"?**
A: Оба варианта законны. Разница:
| | "acknowledge" | "agree" |
|---|---|---|
| Перевод | «Я ознакомлен» | «Я согласен» |
| Юридический смысл | Подтверждение получения информации | Принятие условий договора |
| Когда использовать | Когда нужно ПРОИНФОРМИРОВАТЬ (GDPR Art. 13) | Когда нужно СОГЛАСИЕ (контракт) |
| Для Bestme | ✅ Безопаснее — не создаёт лишних обязательств | ✅ Тоже допустимо — ToS = договор (Art. 6(1)(b)) |
| Рекомендация | **Используем "acknowledge"** | Если юрист попросит — можно заменить |

**Q: Что произойдёт после нажатия кнопки «I understand, continue»?**
A: Последовательность:
1. Если toggle ON → отправить `PATCH /api/users/me { profile_visibility: "public" }`
2. Записать в `legal_consents_log` (что пользователь видел экран + его выбор)
3. Пометить в локальном хранилище: `welcome_screen_shown = true`
4. **Перейти на Feed (лента)** — главный экран приложения
5. Экран больше НИКОГДА не показывается (проверять флаг `welcome_screen_shown`)

**Q: Как пользователь поймёт что это важно?**
A: Экран блокирующий — нельзя закрыть, нет кнопки «назад», нет свайпа. Единственный способ продолжить = нажать кнопку. Это стандартный паттерн onboarding (Pinterest, Instagram, TikTok). Дизайнер должен:
- Сделать заголовок крупным и заметным
- Выделить слово **"private"** жирным шрифтом
- Выделить слово **"always"** жирным шрифтом
- Toggle должен быть визуально заметным (не мелким)
- Кнопка должна быть крупной и контрастной
